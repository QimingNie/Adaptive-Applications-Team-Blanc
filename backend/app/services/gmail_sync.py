...
# def _fetch_message_internal_date(
#     client: httpx.Client, token: str, message_id: str
# ) -> tuple[str, int]:
#     meta = _gmail_get(client, token, f"/messages/{message_id}", params={"format": "metadata"})
#     return (message_id, _internal_date_ms(meta))


# def _list_inbox_message_ids(client: httpx.Client, token: str, pool_size: int) -> list[str]:
#     listing = _gmail_get(
#         client,
#         token,
#         "/messages",
#         params={"labelIds": "INBOX", "maxResults": pool_size},
#     )
...
#         token,
#         "/messages",
#         params={"q": "in:inbox", "maxResults": pool_size},
#     )
...
#         # Gmail's messages.list order is not chronological (often thread-based). Pull a larger
#         # pool, sort by internalDate from lightweight metadata fetches, then full-fetch the newest.
#         # Metadata is fetched in parallel — sequential calls were routinely 1–3+ minutes for large pools.
#         pool_size = min(48, max(max_results * 2, 24))
#         candidates = _list_inbox_message_ids(client, token, pool_size)
#         ranked: list[tuple[str, int]] = []
#         if candidates:
#             workers = min(12, len(candidates))

#             def job(mid: str) -> tuple[str, int]:
#                 # One client per task avoids sharing connection state across threads.
#                 with httpx.Client(timeout=timeout) as thread_client:
#                     return _fetch_message_internal_date(thread_client, token, mid)

#             with ThreadPoolExecutor(max_workers=workers) as pool:
#                 ranked = list(pool.map(job, candidates))
#         ranked.sort(key=lambda row: row[1], reverse=True)
#         message_ids = [mid for mid, _ in ranked[:max_results]]
...
from app.services.adaptation import rescore_user_emails
from app.services.auth import get_valid_access_token
from app.services.ranking import score_email, score_email_explained, bucket_for_score
from app.services.summary import generate_busy_summary
...
def sync_gmail_inbox(db: Session, user: User, max_results: int = 30) -> int:
    ...
    with httpx.Client(timeout=timeout) as client:
        ...
        for message_id in message_ids:
            ...
            _sr = score_email_explained(mail, pref)
            score = _sr.score
            bucket = bucket_for_score(score)
            needs_action = _sr.needs_action
            summary, action_items = generate_busy_summary(mail, allow_llm=False)
            mail.score = score
            mail.bucket = bucket
            mail.needs_action = needs_action
